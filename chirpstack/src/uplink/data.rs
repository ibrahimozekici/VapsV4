...
impl<'a> Data<'a> {
    ...
    async fn send_uplink_event(&mut self) -> Result<()> {
        ...
        if !self._is_end_to_end_encrypted() {
            ...
            .await
            {
                Ok(v) => {
                    match &v {
                        Some(ref obj) => match serde_json::to_string_pretty(obj) {
                            Ok(json) => info!("Decoded objectJson:\n{}", json),
                            Err(e) => warn!("Failed to serialize objectJson: {}", e),
                        },
    ...
    fn _is_relay(&self) -> bool {
        ...
        if let lrwn::Payload::MACPayload(pl) = &self.phy_payload.payload {
            if dp.is_relay.unwrap_or(false) && pl.f_port.unwrap_or(0) == lrwn::LA_FPORT_RELAY {
                return true;
            }
        }
