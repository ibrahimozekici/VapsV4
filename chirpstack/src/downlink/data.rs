...
impl Data {
    ...
    async fn set_mac_commands(&mut self) -> Result<()> {
        ...
        if self.device_profile.is_relay.unwrap_or(false) {
        ...
        if self.device_profile.is_relay_ed.unwrap_or(false) {
                    ...
                    if rd.dev_eui == device.dev_eui.to_vec() {
                        ...
                        if !rd.provisioned
                            ...
                            || rd.uplink_limit_bucket_size
                                != device.relay_ed_uplink_limit_bucket_size.unwrap_or(0) as u32
                            || rd.uplink_limit_reload_rate
                                != device.relay_ed_uplink_limit_reload_rate.unwrap_or(0) as u32
                        {
                ...
                if !found {
                    ...
                    ds.relay
                        ...
                        .push(internal::RelayDevice {
                            ...
                            uplink_limit_bucket_size: device
                                .relay_ed_uplink_limit_bucket_size
                                .unwrap_or(0)
                                as u32,
                            uplink_limit_reload_rate: device
                                .relay_ed_uplink_limit_reload_rate
                                .unwrap_or(0)
                                as u32,
                            provisioned: false,
                ...
                lrwn::ConfigureFwdLimitReqPayload {
                    reload_rate: lrwn::FwdLimitReloadRatePL {
                        ...
                        global_uplink_reload_rate: self
                            ...
                            .relay_global_uplink_limit_reload_rate
                            .unwrap_or(0) as u8,
                        notify_reload_rate: self
                            .device_profile
                            .relay_notify_limit_reload_rate
                            .unwrap_or(0) as u8,
                        join_req_reload_rate: self
                            .device_profile
                            .relay_join_req_limit_reload_rate
                            .unwrap_or(0) as u8,
    ...
    async fn _update_relay_conf(&mut self) -> Result<()> {
        ...
        if relay.enabled != self.device_profile.relay_enabled.unwrap_or(false)
            || relay.cad_periodicity
                != self.device_profile.relay_cad_periodicity.unwrap_or(0) as u32
            || relay.default_channel_index
                != self.device_profile.relay_default_channel_index.unwrap_or(0) as u32
            || relay.second_channel_freq
                != self.device_profile.relay_second_channel_freq.unwrap_or(0) as u32
            || relay.second_channel_dr
                != self.device_profile.relay_second_channel_dr.unwrap_or(0) as u32
            ...
            let set = lrwn::MACCommandSet::new(vec![lrwn::MACCommand::RelayConfReq(
                lrwn::RelayConfReqPayload {
                    channel_settings_relay: lrwn::ChannelSettingsRelay {
                        ...
                        cad_periodicity: self.device_profile.relay_cad_periodicity.unwrap_or(0)
                            as u8,
                        default_ch_idx: self.device_profile.relay_default_channel_index.unwrap_or(0)
                            as u8,
                        second_ch_idx: if self.device_profile.relay_second_channel_freq.unwrap_or(0)
                            > 0
                        {
                            1
                        } else {
                        ...
                        second_ch_dr: self.device_profile.relay_second_channel_dr.unwrap_or(0)
                            as u8,
                        second_ch_ack_offset: self
                            .device_profile
                            .relay_second_channel_ack_offset
                            .unwrap_or(0) as u8,
                    },
                    second_ch_freq: self.device_profile.relay_second_channel_freq.unwrap_or(0)
                        as u32,
                },
            )]);
    ...
    async fn _update_end_device_conf(&mut self) -> Result<()> {
        ...
        // Get the current relay state or default
        let relay = ds.relay.clone().unwrap_or_default();

        // Use extracted and sanitized values
        let ed_activation_mode = self.device_profile.relay_ed_activation_mode.unwrap_or(0) as u32;
        let smart_enable_level =
            self.device_profile.relay_ed_smart_enable_level.unwrap_or(0) as u32;
        let back_off = self.device_profile.relay_ed_back_off.unwrap_or(0) as u32;
        let second_channel_freq = self.device_profile.relay_second_channel_freq.unwrap_or(0) as u32;
        let second_channel_dr = self.device_profile.relay_second_channel_dr.unwrap_or(0) as u32;
        let second_channel_ack_offset = self
            .device_profile
            .relay_second_channel_ack_offset
            .unwrap_or(0) as u32;

        let needs_update = relay.ed_activation_mode != ed_activation_mode
            || relay.ed_smart_enable_level != smart_enable_level
            || relay.ed_back_off != back_off
            || relay.second_channel_freq != second_channel_freq
            || relay.second_channel_dr != second_channel_dr
            || relay.second_channel_ack_offset != second_channel_ack_offset;

        if needs_update {
            let second_ch_idx = if second_channel_freq > 0 { 1 } else { 0 };
            // inside the function
            let raw_mode = self.device_profile.relay_ed_activation_mode.unwrap_or(0);
            // let relay_mode = RelayModeActivation::try_from(raw_mode as u8)
            //     .unwrap_or(RelayModeActivation::Disabled); // or default
            ...
            //         //     relay_mode_activation: relay_mode,
            //         //     smart_enable_level: smart_enable_level as u8,
            //         // },
            //         channel_settings_ed: lrwn::ChannelSettingsED {
            //             second_ch_ack_offset: second_channel_ack_offset as u8,
            //             second_ch_dr: second_channel_dr as u8,
            //             second_ch_idx,
            //             backoff: back_off as u8,
            //         },
            //         second_ch_freq: second_channel_freq,
            //     },
            // )]);
...
mod test {
    ...
    async fn test_update_uplink_list() {
        ...
        let dp_ed = device_profile::create(device_profile::DeviceProfile {
            name: "dp-ed".into(),
            tenant_id: t.id.into(),
            is_relay_ed: Some(true),
            relay_ed_uplink_limit_bucket_size: Some(2),
            relay_ed_uplink_limit_reload_rate: Some(1),
            ..Default::default()
        })
        .await
        .unwrap();
        ...
        let tests = vec![
            Test {
                ...
                device_profile: device_profile::DeviceProfile {
                    ...
                    relay_cad_periodicity: Some(1),
                    relay_default_channel_index: Some(0),
                    relay_second_channel_freq: Some(868300000),
                    relay_second_channel_dr: Some(3),
                    relay_second_channel_ack_offset: Some(2),
                    ..Default::default()
                },
                expected_mac_commands: vec![],
            },
            Test {
                ...
                device_profile: device_profile::DeviceProfile {
                    ...
                    relay_cad_periodicity: Some(1),
                    relay_default_channel_index: Some(0),
                    relay_second_channel_freq: Some(868500000),
                    relay_second_channel_dr: Some(3),
                    relay_second_channel_ack_offset: Some(2),
                    ..Default::default()
                },
    ...
    async fn test_update_end_device_conf() {
        ...
        let tests = vec![
            Test {
                ...
                device_profile: device_profile::DeviceProfile {
                    relay_ed_activation_mode: Some(
                        lrwn::RelayModeActivation::EnableRelayMode as i16,
                    ),
                    relay_ed_smart_enable_level: Some(1),
                    relay_ed_back_off: Some(16),
                    relay_second_channel_freq: Some(868100000),
                    relay_second_channel_dr: Some(3),
                    relay_second_channel_ack_offset: Some(4),
                    ..Default::default()
                },
                expected_mac_commands: vec![],
            },
    ...
    async fn test_configure_fwd_limit_req() {
        ...
        let tests = vec![
            Test {
                ...
                device_profile: device_profile::DeviceProfile {
                    relay_join_req_limit_reload_rate: Some(10),
                    relay_join_req_limit_bucket_size: Some(0),
                    relay_notify_limit_reload_rate: Some(15),
                    relay_notify_limit_bucket_size: Some(1),
                    relay_global_uplink_limit_reload_rate: Some(20),
                    relay_global_uplink_limit_bucket_size: Some(2),
                    relay_overall_limit_reload_rate: Some(25),
                    relay_overall_limit_bucket_size: Some(3),
                    ..Default::default()
                },
                expected_mac_commands: vec![],
            },
            Test {
                ...
                device_session: internal::DeviceSession {
                    relay: Some(internal::Relay {
                        join_req_limit_reload_rate: Some(5),
                        join_req_limit_bucket_size: Some(0),
                        notify_limit_reload_rate: Some(15),
                        notify_limit_bucket_size: Some(1),
                        global_uplink_limit_reload_rate: Some(20),
                        global_uplink_limit_bucket_size: Some(2),
                        overall_limit_reload_rate: Some(25),
                        overall_limit_bucket_size: Some(3),
                        ..Default::default()
                    }),
                    ..Default::default()
                },
